  ...
  return (
    <div className="rounded-xl border border-border bg-card p-4 sm:p-6 shadow-card">
      <div className="mb-6 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        ...
        <div className="flex overflow-x-auto pb-1 sm:pb-0 -mx-1 px-1 sm:mx-0 sm:px-0 no-scrollbar">
          <div className="flex rounded-lg bg-secondary p-1 min-w-max">
            {(Object.keys(periodLabels) as TimePeriod[]).map((p) => (
              <button
                key={p}
                onClick={() => setPeriod(p)}
                className={`rounded-md px-3 py-1.5 text-xs font-medium transition-all whitespace-nowrap ${period === p
